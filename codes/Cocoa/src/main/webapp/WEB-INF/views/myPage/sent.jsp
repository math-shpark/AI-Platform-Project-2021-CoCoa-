	...
	<div class="card rcol my-5"
		...
		<div style="margin: 0 auto; font-size: 30px;">
			...
			<c:if test="${pageMaker.prev }">
				<a
					href='<c:url value="/view_sendReq?page=${pageMaker.startPage-1 }"/>'><i
					class=""></i></a>
			</c:if>

			<c:forEach begin="${pageMaker.startPage }"
				end="${pageMaker.endPage }" var="pageNum">
				<a href='<c:url value="/view_sendReq?page=${pageNum }"/>'><i
					class="">${pageNum }</i></a>
			</c:forEach>

			<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
				<a
					href='<c:url value="/view_sendReq?page=${pageMaker.endPage+1 }"/>'><i
					class=""></i></a>
			</c:if>
